...
Проект находится в активной разработке. Уже доступны web/API-платформа, библиотека резюме и сохранённый legacy AI-workflow. Сценарий вакансии, фоновые jobs, история материалов и HR-сценарии пока запланированы. Это не production deployment: AI не отправляет отклики, письма или сообщения от имени пользователя, а выводы и подготовленные материалы требуют ручной проверки.
...
Подробнее о текущих и целевых компонентах — в [архитектуре](docs/architecture.md). Целевая модель с worker, очередью и сущностью вакансии ещё не реализована.
...
После запуска откройте `http://localhost`. Для mock-разработки установите `LLM_MOCK=true`. Для real LLM заполните `LLM_API_KEY`, `LLM_MODEL` и `LLM_BASE_URL` данными своего провайдера; также нужен локальный private prompt overlay, которого в репозитории нет.

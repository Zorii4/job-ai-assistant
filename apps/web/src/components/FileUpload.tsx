...
export function FileUpload({ id, label, file, onFileChange, disabled, description, previewState }: { id: string; label: string; file: File | null; onFileChange: (file: File | null) => void; disabled?: boolean; description?: string; previewState?: 'hover' | 'focus' | 'active' | 'loading' | 'error' | 'success' }) {
  ...
  return <section className={`file-upload${previewState !== undefined ? ` is-${previewState}` : ''}`} data-state={state} aria-labelledby={`${id}-label`}>
    ...
    <div className="file-upload__content"><div><span id={`${id}-label`} className="sr-only">{label}</span><p id={`${id}-description`} className="file-upload__description">{description ?? (label === 'Файл вакансии' ? 'PDF, MD или TXT · до 10 МБ. Загрузить файл с вакансией.' : 'PDF, MD или TXT · до 10 МБ')}</p></div>{file !== null && <p className="file-upload__file" role="status">{file.name} · {getFileTypeLabel(file)} · {formatFileSize(file.size)}</p>}{errorMessage !== null && <p id={`${id}-error`} className="file-upload__error" role="alert">{errorMessage}</p>}</div>
    <div className="file-upload__actions">{file === null ? <button className="button button--secondary" type="button" onClick={() => inputRef.current?.click()} disabled={isDisabled}>Выбрать файл</button> : <button className="button button--secondary" type="button" onClick={clearFile} disabled={isDisabled}>Удалить</button>}</div>
  </section>;
}

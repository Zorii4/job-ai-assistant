...
const quickStageTransitions: Partial<Record<ApplicationCaseStatus, Array<{ status: ApplicationCaseStatus; label: string }>>> = {
  ...
  HR_INVITED: [{ status: 'REJECTED', label: 'Получен отказ' }, { status: 'ARCHIVED', label: 'Архивировать' }],
  ...
  HR_PREPARATION_READY: [{ status: 'ARCHIVED', label: 'Архивировать' }],
    ...
    <section className="application-case-group" aria-label={title}>
      ...
      <ul className="application-case-cards">
        {cases.map((applicationCase) => {
          ...
          const canEnterPostInterview = applicationCase.postInterviewRun === null && (applicationCase.status === 'HR_INVITED' || applicationCase.status === 'HR_PREPARATION_READY');
          return <li key={applicationCase.id} className="application-case-card"><div><h4>{applicationCase.title}</h4><p className="application-case-card__lifecycle">{applicationStatusLabels[applicationCase.status]}</p>{analysisStatus !== null && <p>{analysisStatus}</p>}{hrPreparationStatus !== null && <p>{hrPreparationStatus}</p>}{applicationCase.postInterviewRun !== null && <p>Разбор после HR: {getAnalysisRunStatusLabel(applicationCase.postInterviewRun.status)}</p>}<p className="application-case-card__dates">Создано: {dateFormatter.format(new Date(applicationCase.createdAt))} · Обновлено: {dateFormatter.format(new Date(applicationCase.updatedAt))}</p></div><div className="application-case-card__actions"><label className="field"><span className="sr-only">Изменить статус</span><select value={applicationCase.status} disabled={isUpdatingStage || isHrPreparationActive || isActiveAnalysisStatus(run?.status ?? 'SUCCEEDED')} onChange={(event) => onUpdateStage(applicationCase.id, event.target.value as ApplicationCaseStatus)}>{Object.entries(applicationStatusLabels).map(([status, label]) => <option key={status} value={status}>{label}</option>)}</select></label>{run !== null && (isFailed ? <button className="button button--secondary button--small" type="button" disabled={isRetrying || isUpdatingStage || isHrPreparationActive} onClick={() => onRetryAnalysis(applicationCase.id)}>{isRetrying ? 'Повторяем…' : 'Повторить анализ'}</button> : <button className="button button--secondary button--small" type="button" disabled={isUpdatingStage || isHrPreparationActive} onClick={() => onOpenAnalysis(applicationCase.id, run.id)}>{applicationCase.status === 'HR_PREPARATION_READY' ? 'Открыть подготовку' : actionLabel}</button>)}{canLaunchHrPreparation && <button className="button button--primary button--small" type="button" disabled={isPreparingHr || isUpdatingStage} onClick={() => onLaunchHrPreparation(applicationCase.id)}>{isPreparingHr ? 'Запускаем…' : hrPreparationRun?.status === 'FAILED' ? 'Повторить подготовку' : 'Подготовиться к HR'}</button>}{canEnterPostInterview && run !== null && <button className="button button--primary button--small" type="button" disabled={isUpdatingStage || isHrPreparationActive} onClick={() => onOpenAnalysis(applicationCase.id, run.id)}>HR-скрининг состоялся</button>}{!isActiveAnalysisStatus(run?.status ?? 'SUCCEEDED') && !isHrPreparationActive && quickStageTransitions[applicationCase.status]?.map((transition) => <button key={transition.status} className="button button--secondary button--small" type="button" disabled={isRetrying || isPreparingHr || isUpdatingStage} onClick={() => onUpdateStage(applicationCase.id, transition.status)}>{isUpdatingStage ? 'Сохраняем…' : transition.label}</button>)}</div></li>;
        })}
      </ul>
    </section>
  );

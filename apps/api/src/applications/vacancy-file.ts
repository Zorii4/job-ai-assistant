...
const MAX_VACANCY_FILE_SIZE_BYTES = 10 * 1024 * 1024;
...
export type UploadedVacancyFile = {
  originalname: string;
  mimetype: string;
  size: number;
  buffer: Buffer;
...
export function filterVacancyUpload(
  _request: unknown,
  file: Pick<UploadedVacancyFile, 'originalname' | 'mimetype'>,
  callback: (error: Error | null, acceptFile: boolean) => void,
): void {
  callback(null, getSupportedExtension(file.originalname, file.mimetype) !== undefined);
}

export async function extractUploadedVacancyFile(
  file: UploadedVacancyFile | undefined,
): Promise<{ sourceFileName: string; sourceText: string }> {
  if (file === undefined || file.size === 0 || file.size > MAX_VACANCY_FILE_SIZE_BYTES) {
  ...
  try {
    const sourceText = extension === '.pdf'
    ...
    return { sourceFileName: getSafeFileName(file.originalname), sourceText };
  } finally {
    file.buffer.fill(0);
  }
}

export function getMaxVacancyFileSizeBytes(): number {
  return MAX_VACANCY_FILE_SIZE_BYTES;
}

function getSupportedExtension(fileName: string, mimeType: string): SupportedExtension | undefined {
